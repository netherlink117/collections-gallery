...
<template>
  ...
  <div v-else class="flex flex-wrap p-3">
    <!--Column-->
    <div v-for="(item, index) of directory.content.directories" v-bind:key="item.path + index"
    ...
    <!--Column-->
    <div v-for="(item, index) of directory.content.files" v-bind:key="item.path + index"
      class="p-3 w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5 xl:w-1/6 2xl:w-1/7 aspect-square">
    <!--Square-->
      <router-link :to="{
        name: 'Viewer',
        ...
        <div v-if="item.mimetype.indexOf('image') >= 0"
          class="w-full h-full bg-center bg-normal hover:bg-hover duration-300 relative"
          :style="'background-image: url(\'' + getFileSrc(item) + '\')'">

          <div class="px-3 py-1 bg-opacity-90 dark:bg-opacity-90 dark:bg-dark-700 bg-light-700 text-emerald-500 bottom-0 left-0 right-0 absolute">
        ...
        <div v-else-if="item.mimetype.indexOf('video') >= 0"
          class="w-full h-full bg-purple-500 hover:bg-purple-700 duration-300 relative">

          <div class="px-3 py-1 bg-opacity-90 dark:bg-opacity-90 dark:bg-dark-700 bg-light-700 text-purple-500 bottom-0 left-0 right-0 absolute">
            <div class="w-full mx-auto truncate">
              {{ item.name }}
            </div>
            <div class="w-full mx-auto truncate">
