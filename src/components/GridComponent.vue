    ...
    <div v-for="(item, index) of directory.content.files" v-bind:key="item.path + index"
      ...
      <router-link :to="{
        ...
        <div v-if="item.mimetype.indexOf('image') >= 0"
          ...
          <div class="w-full bg-dark-900 bg-opacity-75 text-emerald-500 px-1.5 pt-1.5">
            <div class="w-7/10 mx-auto truncate">
              {{ item.name }}
            </div>
          </div>
          <div class="w-full bg-dark-900 bg-opacity-75 text-center text-emerald-500 px-1.5 pb-1.5">
